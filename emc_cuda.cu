...
__global__ void update_slices_kernel(real * images, real * slices, int * mask, real * respons,
				     ...
				     real w, int slice_rows, int slice_cols,
...
real cuda_update_slices(sp_matrix ** images, sp_matrix ** slices, sp_imatrix * mask,
  ...
  cudaMemcpy(d_model,model,sizeof(real)*sp_3matrix_size(model),cudaMemcpyHostToDevice);

  real * d_weight;
  cudaMalloc(&d_weight,sizeof(real)*sp_3matrix_size(weight));
  cudaMemcpy(d_weight,weight,sizeof(real)*sp_3matrix_size(weight),cudaMemcpyHostToDevice);
  ...
  cudaMemcpy(d_x_coordinates,x_coordinates,sizeof(real)*sp_matrix_size(x_coordinates),cudaMemcpyHostToDevice);

  real * d_y_coordinates;
  cudaMalloc(&d_y_coordinates,sizeof(real)*sp_matrix_size(y_coordinates));
  cudaMemcpy(d_y_coordinates,y_coordinates,sizeof(real)*sp_matrix_size(y_coordinates),cudaMemcpyHostToDevice);

  real * d_z_coordinates;
  cudaMalloc(&d_z_coordinates,sizeof(real)*sp_matrix_size(z_coordinates));
  cudaMemcpy(d_z_coordinates,z_coordinates,sizeof(real)*sp_matrix_size(z_coordinates),cudaMemcpyHostToDevice);
  ...
  update_slices_kernel<<<nblocks,nthreads>>>(d_images, d_slices, d_mask, d_respons,
					     ...
					     d_y_coordinates,d_z_coordinates,d_model, d_weight,1.0,
